...
import { close, pencil, trash, search, add, star } from 'ionicons/icons';
...
import { getAllJourneys, getJourneys, toggleJourneyFavoriteBothWays } from '../lib/user';
...
const AllJourneys: React.FC = () => {
  ...
  const [hasFavoriteJourneys, setHasFavoriteJourneys] = useState(false);
  ...
  async function updateJourneys() {
    ...
    const updatedJourneys = await getAllJourneys();
    setAllJourneys(updatedJourneys); // Save unfiltered
    // Check for favorite journeys
    const allUserJourneys = await getJourneys();
    const favCount = allUserJourneys.filter((j: any) => j.favorite).length;
    setHasFavoriteJourneys(favCount);
  ...
  // Show the mark as favourite button if edit mode is enabled and there are journeys
  function showMarkAsFavoriteButton() {
    if (editMode && journeys && journeys.length > 0) {
      return (
        <IonButton expand='block' color='warning' onClick={markAllAsFavorites} className='ion-margin'>
  ...
  // Function to mark all visible journeys as favorites
  async function markAllAsFavorites() {
    const allJourneys = await getJourneys();
    const updatedJourneys = allJourneys.map((journey: any) => {
      // Check if this journey is in the current list (non-favorites)
      const isInCurrentList = journeys.some((currentJourney: any) => 
        currentJourney.from === journey.from && currentJourney.to === journey.to
      );
      if (isInCurrentList) {
        return { ...journey, favorite: true };
      }
      return journey;
    });
  ...
  // Show the delete all journeys button if edit mode is enabled and there are journeys
  ...
  // Function to delete all journeys
  async function deleteAllJourneys() {
    await setPreferenceJson('journeys', []);
    window.dispatchEvent(new CustomEvent('journeysChanged'));
    updateJourneys(); // Refresh the list
    ...
    if (!updateInProgress && (!journeys || journeys.length === 0)) {
      return (
        <IonCard>
          ...
          <IonCardContent>
            <p className='ion-text-center'>No saved journeys found. Add a journey to see it here...</p>
            <TrainImage />
    ...
    if (filteredOut > 0) {
      return (
        <div className="ion-text-center ion-padding-top subtle-info-link">
            {filteredOut} journey{filteredOut > 1 ? 's are' : ' is'} hidden by your distance filter.{' '}
            <br />
            <a
              href="#"
              onClick={async (e) => {
  ...
  function showNoMyJourneysLink() {
    const favCount = hasFavoriteJourneys;
    if (!updateInProgress && typeof favCount === 'number' && favCount > 0) {
      return (
        <div className="ion-text-center ion-padding-top subtle-info-link">
          You have {favCount} favourite journey{favCount === 1 ? '' : 's'}.{' '}
          <br />
          <a href="/favourites" className="filtered-info-link">View my favourite journey{favCount === 1 ? '' : 's'}</a>
        </div>
      );
    }
    return null;
  ...
  return (
    <IonPage>
      ...
      <IonContent fullscreen className="alljourneys-content-scroll">
        {/* Add Journey FAB */}
        <IonFab slot="fixed" vertical="top" horizontal="end" edge={true} style={{ top: '5px', right: '16px', zIndex: 11 }}>
        ...
        <IonHeader collapse="condense">
          <IonToolbar>
            <IonTitle size="large">My Journeys</IonTitle>
          </IonToolbar>
        </IonHeader>
        ...
        {/* Mark all as favorites button */}
        {showMarkAsFavoriteButton()}

        {/* Delete all journeys button */}
        {showDeleteAllButton()}
        ...
        {/* Show update in progress indicator at the bottom */}
        {showUpdateInProgress()}

        {showFilteredOutMessage()}

        {showNoMyJourneysLink()}
        ...
        <IonAlert
          ...
          message="Are you sure you want to delete ALL journeys? This action cannot be undone."
          buttons={[
            {
              text: 'Cancel',
              role: 'cancel',
